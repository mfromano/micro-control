...
void sendData(time_t capturetime, uint8_t usage);
...
void capture() {
  ...
  while (capture_t < 1100) {
    ;
  }
  fastDigitalWrite(CAMERA_PIN,LOW);

  temp_t = capture_t;
  sendData(temp_t, AudioProcessorUsage());
}

void sendData(time_t capturetime, uint8_t usage) {
  String exp_time = String(capturetime);
  String newtime = String(usage);
  Serial.println( exp_time + delimiter + newtime);
}
