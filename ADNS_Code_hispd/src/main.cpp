...
volatile int32_t movementFrameCount;
volatile time_t currentFrameTimestamp;
volatile uint32_t currentFrameCount;
volatile int32_t nreps = 0;
volatile bool isRunning = false;
volatile bool camera_on = false;
volatile int32_t sampling_interval_ms_int;
const uint16_t movementRate = 1000; // 1000 microseconds per sample
// =============================================================================
//   SETUP & LOOP
// =============================================================================
...
static inline void beginAcquisition(char input[], int8_t length) {
    ...
    sampling_interval_ms_int = atof(sampling_interval_ms);
...
void captureDisplacement() {
  ...
  // Send Data
  sendData(currentSample,camera_on);
...
void sendData(sensor_sample_t sample, bool camera_pulse) {
