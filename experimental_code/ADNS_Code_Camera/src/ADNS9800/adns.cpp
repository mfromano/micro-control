...
void ADNS::triggerPixelStart() {
  noInterrupts();
  // first write 0x5a to Power_Up_Reset (0x3a)
  SPI.transfer((byte)RegisterAddress::Power_Up_Reset, 0x5a);
  // set Forced_Disable bit of laser ctrl to 0
  uint8_t laser_ctrl = readRegister((byte)RegisterAddress::LASER_CTRL0);
  writeRegister((byte)RegisterAddress:LASER_CTRL0, laser_ctrl & 0xfe);

  interrupts();
}

void ADNS::triggerPixelCapture() {
  if (!_pixelFlag) triggerPixelStart();

  SPI.transfer((byte)RegisterAddress::Frame_Capture, 0x93);
  SPI.transfer((byte)RegisterAddress::Frame_Capture, 0xc5);
  while (_microsSinceCapture < 2*_minSamplePeriodUs) {
    yield;
  }
