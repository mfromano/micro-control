...
void sendHeader() {
  ...
  Serial.print(String(
      ...
      flatFieldNames[2] + " [" + tunit + "]" + delimiter + "left motion" + delimiter + flatFieldNames[4] + " [" + dunit + "]" +
      delimiter + flatFieldNames[5] + " [" + dunit + "]" + delimiter + flatFieldNames[6] + " [" + tunit + "]" +
      delimiter + "right motion" + delimiter + " waterPin " "\n"));
}
...
void sendData(sensor_sample_t sample, bool waterPin) {
    ...
    const String motionL = String(sample.left.p.motion);
    ...
    const String motionR = String(sample.right.p.motion);
    ...
    Serial.print(timestamp + delimiter + dxL + delimiter + dyL + delimiter +
                 dtL + delimiter + motionL + delimiter + dxR + delimiter + dyR + delimiter + dtR + delimiter + motionR + delimiter + waterPinVal +
                 endline);
}
