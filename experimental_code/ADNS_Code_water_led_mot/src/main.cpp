...
void sendHeader() {
  ...
  Serial.print(String(
      ...
      flatFieldNames[2] + " [" + tunit + "]" + delimiter + "motL" + delimiter + flatFieldNames[3] +
      " [" + dunit + "]" + delimiter + flatFieldNames[4] + " [" + dunit + "]" +
      delimiter + flatFieldNames[5] + " [" + tunit + "]" + delimiter + "motR" + delimiter + " waterPin " "\n"));
}
...
void sendData(sensor_sample_t sample, bool waterPin) {
    ...
    const String xL = String(sample.left.p.dx, decimalPlaces);
    const String yL = String(sample.left.p.dy, decimalPlaces);
    const String tL = String(sample.left.p.dt, decimalPlaces);
    const String motL = String(sample.left.p.mot);
    const String xR = String(sample.right.p.dx, decimalPlaces);
    const String yR = String(sample.right.p.dy, decimalPlaces);
    const String tR = String(sample.right.p.dt, decimalPlaces);
    const String motR = String(sample.right.p.mot);
    ...
    Serial.print(timestamp + delimiter + xL + delimiter + yL + delimiter +
                 tL + delimiter + motL + delimiter + xR + delimiter + yR + delimiter + tR + delimiter +motR+delimiter+ waterPinVal +
                 endline);
}
