...
void sendHeader() {
  ...
  Serial.print(String(
      ...
      flatFieldNames[2] + " [" + tunit + "]" + delimiter + "shutterLL" +
      + delimiter + "shutterHL" + flatFieldNames[4] + " [" + dunit + "]" +
      delimiter + flatFieldNames[5] + " [" + dunit + "]" + delimiter + flatFieldNames[6] + " [" + tunit + "]" +
      delimiter + "shutterLR" + delimiter + "shutterHR" + delimiter + " waterPin " "\n"));
}
...
void sendData(sensor_sample_t sample, bool waterPin) {
    ...
    const String shutterLL = String(sample.left.p.shutterL);
    const String shutterHL = String(sample.left.p.shutterH);
    ...
    const String shutterLR = String(sample.right.p.shutterL);
    const String shutterHR = String(sample.right.p.shutterH);

    // Serial.availableForWrite
    // Print ASCII Strings
    Serial.print(timestamp + delimiter + dxL + delimiter + dyL + delimiter +
                 dtL + delimiter + shutterLL + delimiter + shutterHL +delimiter + dxR + delimiter + dyR + delimiter + dtR + delimiter + shutterLR + delimiter + shutterHR + delimiter + waterPinVal +
                 endline);
}
