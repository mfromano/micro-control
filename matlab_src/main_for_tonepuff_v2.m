...
%%
fi = fopen('micro-control-data/MikeTest_9-26-2018/mike_tonepuff2_practice3.txt');
...
save('micro-control-data/tone_puff_table_v2.mat','tbl');

%%

load('micro-control-data/tone_puff_table_v2.mat','tbl');
timestamps_orig = tbl.Time;
trial_timestamps = [1; find([0; diff(tbl.ExpNo) == 1]); length(tbl.ExpNo)+1];

d = 'micro-control-data/MikeTest_9-26-2018/Block-2';
data = TDTbin2mat(d);
